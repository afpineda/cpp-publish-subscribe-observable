...
$_compiler = "g++"
$lib_path = Join-Path $RootPath "/src"
$test_path = Join-Path $RootPath "/test"
$_compiler_args = @(
    "-fdiagnostics-color=always", # colored output
    "-std=c++20", # C++ standard revision 20
    "-iquote",
    $lib_path, # Includes path
...
function Solve-SourceFile {
    param(
        ...
        [string]$BuildPath)
    process {
        $candidate = Join-Path $BuildPath $FileName
...
if (-not (Test-Path $test_path)) {
    throw "❌ Error. '$test_path' folder not found"
}
...
foreach ($buildFile in $buildFiles) {
    ...
    if (Test-IsRequired $buildFolder) {
        ...
        $sourceFiles = $sourceFiles | Solve-SourceFile -BuildPath $buildFolder
